...
class ReflectionAndRefractionTestWindow extends JOGLFWindow{
	private int env_mapping_texture_id;
	...
	private void GenerateCubemap() {
		...
		env_mapping_texture_id=texture_ids.get(0);
		...
		GLWrapper.glBindTexture(GL4.GL_TEXTURE_CUBE_MAP, env_mapping_texture_id);
		for(int i=0;i<6;i++) {
	...
	private void SetupProgram() {
		GLShaderFunctions.CreateProgram(
				...
				"./Data/Shader/330/reflection_and_refraction/vshader.glsl", 
				"./Data/Shader/330/reflection_and_refraction/fshader.glsl");
		program=new ShaderProgram("reflection_and_refraction");
		program.Enable();
		program.SetUniform("apply_texture", 1);
		program.SetUniform("eta", 1.0f/1.33f);
		
		CameraFront.AddProgram("reflection_and_refraction");
	...
	protected void Draw() {
		...
		Model3D.DrawModelWithProgram(model_handle, "reflection_and_refraction", 0, "texture_sampler");
