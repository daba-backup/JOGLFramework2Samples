...
vec4 GetTexelFromInputTexture(ivec2 p){
    vec2 uv;
...
void HorizontalButterflies(){
    ...
    vec2 p_=GetTexelFromInputTexture(ivec2(data.z,x.y)).rg;
    //Get the values of the bottom butterfly.
    vec2 q_=GetTexelFromInputTexture(ivec2(data.w,x.y)).rg;
    //Twiddle factor
    ...
    fs_out_color=vec4(H.re,H.im,0.0,1.0);
}
void VerticalButterflies(){
    ...
    //Get the values of the top butterfly.
    vec2 p_=GetTexelFromInputTexture(ivec2(x.x,data.z)).rg;
    //Get the values of the bottom butterfly.
    vec2 q_=GetTexelFromInputTexture(ivec2(x.x,data.w)).rg;
    //Twiddle factor
    vec2 w_=vec2(data.x,data.y);
